...
function Cart(props) {
    ...
    useEffect(() => {
        ...
        if (!itemMetaData) {
            ...
            cartItems.forEach((v, i) => {
                promises.push(
                    fetch(
                        `https://fakestoreapi.com/products/${v.productId}`
                    ).then(async response => {
        ...
        const mergedCartData = cartItems.map(v => ({
            ...v,
            ...itemMetaData.find(m => m.productId === v.productId)
        }));
    ...
    const handleUpdateQuantity = (productId, quantity) => {
        ...
        setCartItems(
            cartItems.map(v =>
                v.productId === productId ? { ...v, quantity } : v
            )
        );
    };
    ...
    return (
        <section className="container mx-auto">
            ...
            {!items.length ? (
                ...
                items.map(v => (
                    <div
                        className="flex items-center px-8 py-4 hover:shadow"
                        key={v.productId}
                    >
                        <NavLink
                        ...
                        >
                            <img
                                className="flex-none object-contain h-16 w-16"
                                src={v.image}
                                alt={v.title}
                            />
                            <p className="pl-8 flex-grow">{v.title}</p>
                        </NavLink>
                        <p className="flex-none w-24 text-right">
                            ${v.price.toFixed(2)}
                        </p>
                        <p className="flex-none w-32 text-center flex place-content-center">
                            ...
                            <input
                                ...
                                className="text-center w-8 h-8 border-t border-b"
                                type="text"
                                value={v.quantity}
                            />
                            <button
                        ...
                        <button
                            onClick={() => handleRemoveItem(v.productId)}
                            className="ml-4"
                        >
                            <TrashIcon className="flex-none w-6 h-6 m-2" />
                        </button>
                    </div>
                ))
            )}
